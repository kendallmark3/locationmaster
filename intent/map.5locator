I checked it. Your instinct was right: the intent box is being saved and then used by the Anthropic-powered relocation narrative, but it is not currently driving the map-building behavior itself. The nearby categories are still hard-coded, and the automatic location flow always fetches coffee, restaurants, schools, and parks regardless of what the user typed. 

The marker groundwork is also already there: category-specific colors exist, but the renderer is basically creating the same circular <div> marker for everything, with only color and the larger subject size distinguishing them.  And the backend already has an /export boundary, but it only validates the project and explicitly says the renderer is the next slice—so there is no actual JPEG/PNG download yet. 

So v0.5 should connect those unfinished pieces rather than rebuilding the system. I’d drop the following into intent/PHASE-05-EXPERIENCE.md and tell Claude Code to implement it.
